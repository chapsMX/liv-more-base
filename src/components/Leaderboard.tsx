...
// badges para diferenciar usuarios de farcaster y baseapp
...
function PlatformBadge({ authType, og }: { authType?: string | null; og: boolean }) {
  return (
    <span className="inline-flex items-center gap-0.5">
      ...
      {og && <span className="text-amber-400 text-xs" title="OG">◆</span>}
    </span>
  );
}
              ...
              {monthlyData.map((row) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    ? (displayNameMap[row.fid ?? 0] ?? getDisplayName(row))
                    : (displayNameMap[row.fid ?? 0] ?? getDisplayName(row))
                    }
                    {" "}
        ...
        {feedData.map((row) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-white font-medium truncate">
                {row.fid
                  ? (displayNameMap[row.fid] ?? getDisplayName(row))
                  : getDisplayName(row)}
                {" "}
                <PlatformBadge authType={row.auth_type} og={row.og} />
              </p>
